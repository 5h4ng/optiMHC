...
class PsmContainer:
    ...
    def __init__(
            ...
            for key, cols in features.items():
                for col in cols:
                    if col not in psms.columns:
                        raise ValueError(f"Column '{col}' not found in PSM data for feature '{key}'.")
        ...
        if self.ms_data_file_column:
            logger.info("PSMs originated from %d MS data file(S).", len(self._psms[ms_data_file_column].unique()))
    ...
    def identifier_columns(self) -> List[str]:
        ...
        """
        return [self.scan_column, self.spectrum_column, self.peptide_column, self.protein_column]
    ...
    def drop_features(self, features: List[str]) -> None:
        ...
        logger.info(f'Sources to be removed: {sources_to_update}. Because all the features are removed.')
    ...
    def add_metadata(
        ...
        merged_df = self.psms.merge(metadata_df, left_on=psms_key, right_on=metadata_key, how='left')
        ...
        for col in metadata_cols:
            merged_df['metadata'] = merged_df.apply(
                lambda row: {**row['metadata'], source: {col: row[col]} if source not in row['metadata'] else {**row['metadata'][source], col: row[col]}},
                axis=1
            )
    ...
    def add_features(
        ...
        features_df = features_df.rename(columns={col: col+suffixes[1] for col in new_feature_cols})
        ...
        if feature_key != psms_key:
            cols_to_drop = [col for col in feature_key if col not in psms_key and col in self._psms.columns]
            if cols_to_drop:
        ...
        if len(self._psms) != original_len:
            raise ValueError("Merging features resulted in a change in the number of PSMs. Check for duplicate keys.")
    ...
    def add_features_by_index(
        ...
        self.rescoring_features[source] = [col + suffixes[1] for col in new_feature_cols]
        features_df.rename(columns={col: col + suffixes[1] for col in new_feature_cols}, inplace=True)
        original_len = len(self._psms)
        self._psms = self._psms.merge(
            features_df,
            left_index=True,
        ...
        if len(self._psms) != original_len:
            raise ValueError("Merging features resulted in a change in the number of PSMs. Check for duplicate indices.")
    ...
    def add_results(
        ...
        if set(self._psms.columns) & set(results_df.columns):
            raise ValueError("Duplicate columns exist. Please rename the columns in the results data.")
    ...
    def write_pin(self, output_file: str, source: List[str] = None) -> None:
        ...
        pin_df["Proteins"] = df[self.protein_column].apply(lambda x: ";".join(x) if isinstance(x, (list, tuple)) else x)
