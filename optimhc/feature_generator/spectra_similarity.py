...
class SpectraSimilarityFeatureGenerator(BaseFeatureGenerator):
    ...
    def feature_columns(sself) -> List[str]:
        ...
        """
        return ['spectral_angle_similarity', 'cosine_similarity', 'pearson_correlation', 'spearman_correlation',
                'mean_squared_error', 'unweighted_entropy_similarity', 'predicted_seen_nonzero',
                'predicted_not_seen']
    ...
    def _sort_spectrum_by_mz(self, mz: List[float], intensity: List[float], 
                            annotation: Optional[List[str]] = None) -> Tuple[np.ndarray, np.ndarray, Optional[np.ndarray]]:
        """
    ...
    def _align_spectra_all_peaks(self, exp_mz: List[float], exp_intensity: List[float], 
                              pred_mz: List[float], pred_intensity: List[float],
                              pred_annotation: Optional[List[str]] = None,
                              use_ppm: bool = True) -> Tuple[np.ndarray, np.ndarray, List[Tuple], Dict]:
        """
        Align experimental and predicted spectra using an efficient incremental search
        algorithm similar to MSBooster. This function considers all peaks in the prediction.
        ...
        if pred_annotation is not None:
            pred_mz_sorted, pred_intensity_sorted, pred_annotation_sorted = self._sort_spectrum_by_mz(
                pred_mz, pred_intensity, pred_annotation
            )
        else:
            pred_mz_sorted, pred_intensity_sorted, pred_annotation_sorted = self._sort_spectrum_by_mz(
    ...
    def _get_top_peaks_vectors(self, aligned_exp_intensity: np.ndarray, 
                              aligned_pred_intensity: np.ndarray,
                              matched_indices: List[Tuple], 
                              top_n: int) -> Tuple[np.ndarray, np.ndarray, List[Tuple]]:
        """
    ...
    def _align_spectra(self, exp_mz: List[float], exp_intensity: List[float], 
                     pred_mz: List[float], pred_intensity: List[float],
                     pred_annotation: Optional[List[str]] = None) -> Tuple[np.ndarray, np.ndarray, List[Tuple]]:
        """
        ...
        all_exp_intensity, all_pred_intensity, all_matched_indices, additional_info = self._align_spectra_all_peaks(
            exp_mz, exp_intensity, pred_mz, pred_intensity, pred_annotation, use_ppm=True
        )
    ...
    def _calculate_spectral_angle_similarity(self, exp_vector: np.ndarray, pred_vector: np.ndarray) -> float:
        """
    ...
    def _calculate_cosine_similarity(self, exp_vector: np.ndarray, pred_vector: np.ndarray) -> float:
        """
    ...
    def _calculate_spearman_correlation(self, exp_vector: np.ndarray, pred_vector: np.ndarray) -> float:
        """
    ...
    def _calculate_pearson_correlation(self, exp_vector: np.ndarray, pred_vector: np.ndarray) -> float:
        """
    ...
    def _calculate_mean_squared_error(self, exp_vector: np.ndarray, pred_vector: np.ndarray) -> float:
        """
    ...
    def _calculate_unweighted_entropy_similarity(self, exp_vector: np.ndarray, pred_vector: np.ndarray) -> float:
        """
        ...
        s_mixed = self._calculate_entropy(0.5 * (sum_normalized_exp_vector + sum_normalized_pred_vector))
    ...
    def _calculate_predicted_counts(self, exp_vector: np.ndarray, pred_vector: np.ndarray) -> Tuple[int, int]:
        """
    ...
    def _calculate_bray_curtis_similarity(self, exp_vector: np.ndarray, pred_vector: np.ndarray) -> float:
        """
    ...
    def _calculate_similarity_features(self, exp_vector: np.ndarray, pred_vector: np.ndarray) -> Dict[str, float]:
        """
        ...
        """
        spectral_angle_similarity = self._calculate_spectral_angle_similarity(exp_vector, pred_vector)
        ...
        unweighted_entropy_similarity = self._calculate_unweighted_entropy_similarity(exp_vector, pred_vector)
        predicted_seen_nonzero, predicted_not_seen = self._calculate_predicted_counts(exp_vector, pred_vector)
    ...
    def _generate_features(self) -> pd.DataFrame:
        ...
        if not exp_spectra_df.empty:
            psm_df = pd.merge(psm_df, exp_spectra_df, on=['scan', 'mz_file_path', 'charge'], how='inner', validate='m:1')
        else:
            logger.error("Could not extract experimental spectral data, cannot continue processing")
            return pd.DataFrame()
        ...
        for _, row in psm_df.iterrows():
            ...
            similarity_features = self._calculate_similarity_features(top_exp_intensity, top_pred_intensity)
            result = {
            ...
            if additional_info['pred_annotation_sorted'] is not None:
                result['pred_annotation_sorted'] = additional_info['pred_annotation_sorted'].tolist()
        ...
        result_columns = [
            ...
            'exp_vector', 'pred_vector', 'matched_indices',
            'exp_top_vector', 'pred_top_vector', 'top_matched_indices',
            'spectral_angle_similarity', 'cosine_similarity', 'pearson_correlation', 'spearman_correlation', 
            'mean_squared_error', 'unweighted_entropy_similarity', 'predicted_seen_nonzero',
            'predicted_not_seen'
        ]
        ...
        logger.info(f"Successfully generated spectral similarity features for {len(result_df)} PSMs")
        return result_df
