...
def _align_spectra_numba(
    ...
    pred_intensity_sorted: np.ndarray,
    tolerance_ppm: float
) -> Tuple[np.ndarray, np.ndarray, List[Tuple[int, int]]]:
    """
    Align experimental and predicted spectra using numba for performance optimization.

    This function matches experimental peaks to predicted peaks within a specified
    mass tolerance (in ppm). For each predicted peak, it finds the experimental
    peak with the highest intensity within the tolerance window.

    Parameters
    ----------
    exp_mz_sorted : np.ndarray
        Sorted experimental m/z values in ascending order.
    exp_intensity_sorted : np.ndarray
        Experimental intensity values corresponding to exp_mz_sorted.
    pred_mz_sorted : np.ndarray
        Sorted predicted m/z values in ascending order.
    pred_intensity_sorted : np.ndarray
        Predicted intensity values corresponding to pred_mz_sorted.
    tolerance_ppm : float
        Mass tolerance for peak matching in parts per million (ppm).

    Returns
    -------
    aligned_exp_intensity : np.ndarray
        Aligned experimental intensity array with same length as predicted array.
        Contains the matched experimental intensities for each predicted peak.
        Zero intensity if no experimental peak is found within tolerance.
    pred_intensity_sorted : np.ndarray
        The input predicted intensity array (returned unchanged).
    matched_indices : List[Tuple[int, int]]
        List of tuples containing matching indices. Each tuple contains:
        (predicted_peak_index, experimental_peak_index). If no match is found,
        experimental_peak_index is -1.

    Notes
    -----
    This function is optimized using numba's @njit decorator for fast execution.
    The algorithm uses a sliding window approach to efficiently search for matches
    without redundant comparisons.

    The tolerance window for each predicted peak is calculated as:
    [pred_mz * (1 - tolerance_ppm/1e6), pred_mz * (1 + tolerance_ppm/1e6)]

    When multiple experimental peaks fall within the tolerance window, the one
    with the highest intensity is selected.

    Examples
    --------
    >>> exp_mz = np.array([100.0, 200.0, 300.0])
    >>> exp_int = np.array([50.0, 100.0, 75.0])
    >>> pred_mz = np.array([100.1, 250.0])
    >>> pred_int = np.array([60.0, 80.0])
    >>> aligned_exp, pred_out, matches = _align_spectra_numba(
    ...     exp_mz, exp_int, pred_mz, pred_int, 20.0
    ... )
    >>> aligned_exp
    array([50.0, 0.0])  # First peak matched, second not found
    """
