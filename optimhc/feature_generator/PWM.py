...
class PWMFeatureGenerator(BaseFeatureGenerator):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.pwms: Dict[str, Dict[int, pd.DataFrame]] = self._load_pwms()  # Dict[allele, Dict[mer, pd.DataFrame]]

        # logger all the pwms
        for allele, pwms in self.pwms.items():
            for mer, pwm in pwms.items():
                logger.debug(f'Loaded PWM for allele {allele}, length {mer}: {pwm.shape[1]} positions')
                logger.debug(pwm)
        ...
        logger.info(f"Initialized PWMFeatureGenerator with {len(peptides)} peptides and alleles: {alleles}")
            ...
            if self.mhc_class == 'I':
                for pwm_file in pwm_files_list:
                    ...
                    if mer is not None:
                        ...
                        logger.error(f"Mer length not found in PWM file name: {pwm_file}. Assuming the tailing number is the mer length.")
                ...
                if len(pwm_files_list) == 1:
                    ...
                    logger.error(f"Expected 1 PWM file for allele {allele}, found {len(pwm_files_list)}: {pwm_files_list}")
                    raise ValueError(f"Expected 1 PWM file for allele {allele}, found {len(pwm_files_list)}: {pwm_files_list}")

        logger.info(f"Default PWM file paths set for alleles: {self.alleles}")
        return pwm_files
    

    def _most_conserved_postions(
        self,
        pwm: pd.DataFrame,
        n: int = 2
    ) -> List[int]:
        ...
        """
        if n > pwm.shape[1]:
            raise ValueError(f"Number of positions to return ({n}) exceeds the PWM length ({pwm.shape[1]}).")
    ...
    def _load_pwms(self) -> Dict[str, Dict[int, pd.DataFrame]]:
        ...
        for allele, mer_files in allele_pwm_files.items():
            ...
            for mer, file_path in mer_files.items():
                ...
                try:
                    pwm = pd.read_csv(file_path, delim_whitespace=True, header=None, index_col=0)
                    pwm.columns = [f'{pos+1}' for pos in range(pwm.shape[1])]
                    pwms[allele][mer] = pwm
                    logger.info(f"Loaded PWM for allele {allele}, length {mer} from {file_path}")
    ...
    def _cal_PWM_score_II(
        ...
        for start_idx in range(len(peptide) - core_len + 1):
            ...
            try:
                tmp_score = sum(
                    float(pwm.loc[aa, str(i + 1)])
                    for i, aa in enumerate(subpeptide_9)
                )
            except KeyError as e:
                logger.error(
                    f"Residue '{e}' not found in PWM for allele={allele}, "
        ...
        if self.mhc_class == 'I':
            ...
            if peptide_len < min_mer or peptide_len > max_mer:
                ...
                try:
                    ...
                    logger.error(f"Position '{e}' not found in PWM for allele {allele} and peptide {peptide} with length {peptide_len}.")
    ...
    def generate_features(self) -> pd.DataFrame:
        ...
        if self.remove_pre_nxt_aa:
            features_df['clean_peptide'] = features_df['Peptide'].apply(utils.remove_pre_and_nxt_aa)
        if self.remove_modification:
            features_df['clean_peptide'] = features_df['clean_peptide'].apply(utils.remove_modifications)

        # Convert nonstandard amino acids: U -> C
        features_df['clean_peptide'] = features_df['clean_peptide'].apply(lambda x: x.replace('U', 'C'))
        
        for allele in self.alleles:
            logger.info(f"Generating PWM scores for allele: {allele}, total peptides: {len(features_df)}")
            ...
            if self.mhc_class == 'I':
                ...
                logger.info(f"Missing PWM scores for {na_count} peptides. Using median for imputation.")
                features_df.fillna(
                ...
                if self.anchors != 0:
                    logger.info(f"Generating anchor scores for allele: {allele}, total peptides: {len(features_df)}")
                    anchor_dict = {}
                    min_mer = min(self.pwms[allele].keys())
                    max_mer = max(self.pwms[allele].keys())
                    for mer_len in range(min_mer, max_mer + 1):
                        anchor_dict[mer_len] = self._most_conserved_postions(self.pwms[allele][mer_len], self.anchors)  
                    ...
                    logger.info(f"Missing anchor scores for {na_count} peptides. Using median for imputation.")
                    features_df.fillna(
                ...
                features_df[[f'PWM_Score_{allele}', 
                             f'N_Flank_PWM_Score_{allele}', 
                             f'C_Flank_PWM_Score_{allele}']] = features_df['clean_peptide'].apply(
                    lambda pep: pd.Series(self._cal_PWM_score(pep, allele))
                )
                
                # Impute missing (NaN) with medians for each new column
                for col in [f'PWM_Score_{allele}', f'N_Flank_PWM_Score_{allele}', f'C_Flank_PWM_Score_{allele}']:
                    na_count = features_df[col].isna().sum()
                    logger.info(f"Missing {col} for {na_count} peptides. Using median for imputation.")
